...
client = InferenceClient(
   ...
   token="your_token"  # Replace with your actual Hugging Face token
)
...
def nl_2_mongodb_query(natural_language, data):
    try:
...
data = st.selectbox("Choose which dataset to access:", ["Pixar Films", "Stolen Vehicles"])
    ...
    if not user_input.strip():
        ...
        with st.spinner("Translating..."):
            if db_type == "SQL":
                ...
                query = nl_2_mongodb_query(user_input, data).strip()
